...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	if (!player_info.fakeplayer && g_ctx.local()->is_alive() && e->m_iTeamNum() != g_ctx.local()->m_iTeamNum() && !g_cfg.legitbot.enabled)
	{
		...
		if (e->m_vecVelocity().Length2D() > 29.f && e->m_vecVelocity().Length2D() < 210.f)
		{
			switch (g_ctx.globals.missed_shots[e->EntIndex()] % 5)
			{
			case 0:
		...
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
		//memcpy(player_resolver[e->EntIndex()].m_resolve_layers[0], e->get_animlayers(), e->animlayer_count() * sizeof(AnimationLayer));
		...
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
		//memcpy(player_resolver[e->EntIndex()].m_resolve_layers[2], e->get_animlayers(), e->animlayer_count() * sizeof(AnimationLayer));
		...
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
		//memcpy(player_resolver[e->EntIndex()].m_resolve_layers[1], e->get_animlayers(), e->animlayer_count() * sizeof(AnimationLayer));
	}
