...
#if RELEASE
#if BETA
#define VERSION crypt_str("psense [beta] | ")
#else
#define VERSION crypt_str("psense [alpha] | ")
#endif
#else
#define VERSION crypt_str("psense | ")
#endif
...
void misc::watermark()
{
	...
	auto watermark = crypt_str("psense | ") + g_ctx.username /*+ crypt_str(" | ")*/ + g_ctx.globals.time;
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			watermark = crypt_str("psense | ") + g_ctx.username /*+ crypt_str(" | ")*/ + server + crypt_str(" | ") + std::to_string(g_ctx.globals.ping) + crypt_str(" ms | ") + tickrate + crypt_str(" tick | ") + g_ctx.globals.time;
		}
	}
...
void misc::ChatSpamer()
{
	...
	{ 
		crypt_str("psense - superior paste."),
		crypt_str("get good get psense."),
		crypt_str("im using psense and you are using your trash otc."),
	};
